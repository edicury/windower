...
description: |
  ...
  list_sessions). Use whenever the user asks to record, capture,
  screen-record, or make a video/demo/screencast of an app, browser,
...
4. **Perform the actions being demoed** — click, type, run commands, navigate — whatever the demo is about. Do it with your own tools (see "Driving the UI" below). *You* sequence this step between start and stop.
...
Windower gives you **independent capabilities**, not a workflow. Capture
(`start_recording`/`stop_recording`) and control (`resize_window`) are peers,
and neither owns or even knows about the other. **You** sequence them, and
**you always drive** — Windower has no driving loop of its own, and no code
that synthesizes input or decides what to click. The recording does not know or
care what is driving the screen — you, a human, or nothing at all; nothing in
a session record says.

Use whichever of your own tools matches the surface:

- **Anything in a browser tab** — use the `claude-in-chrome` skill's tools
  (`navigate`, `computer`, `find`, `read_page`, `form_input`, …). They're
  DOM-aware rather than pixel-guessing, so prefer them over screenshot-driven
  input for any step that happens inside a browser window.
- **Native / desktop UI** — use Claude's computer-use tool (screenshot +
  coordinate-based clicks/keys) when the environment you're running in
  provides one.
- **Claude Code itself does not ship a built-in computer-use tool for native
  macOS UI.** If you're driving a native app from inside Claude Code and have
  no computer-use-capable environment available, fall back to your own
  scripting — e.g. `osascript`/System Events via the `Bash` tool — to click
  through and type into the app being demoed.

Whichever tool you use, it's ordinary step-4 work between `start_recording`
and `stop_recording` — no special handshake with Windower is involved, and
nothing about the recording changes based on how you drove it.
