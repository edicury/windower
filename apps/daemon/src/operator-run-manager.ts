...
import type { SessionManager, SidecarFactory, SidecarHandle } from "./session-manager.js";

/**
 * The daemon half of Phase 19. `OperatorRunManager` owns the `OperatorRun`
 ...
 * `OperatorRunStore` exactly as `SessionManager` does for `RecordingSession`,
...
export interface OperatorRunManagerOptions {
...
export class OperatorRunManager {
  ...
  constructor(options: OperatorRunManagerOptions) {
   ...
   * `SessionManager.recoverCrashedSessions()`; call once at startup, after
  ...
  async runOperator(
    ...
    try {
      secrets = await this.secretResolver.resolveAll(params.secrets ?? []);
    } catch (err) {
   ...
   * run keeps whatever the loop reported, matching `SessionManager`'s
  ...
  private async finalize(
    ...
    if (active?.sessionId) {
      try {
        ...
        console.error(`[OperatorRunManager] stopping recording for run ${runId} failed:`, err);
      }
    }
