...
export class SecretResolver {
   ...
   *
   * `forwarded` is the calling connection's `hello`-time env snapshot
   * (`RequestContext.resolvedSecrets`, `contracts/daemon-rpc.md`'s `env`
   * scoping rules) — `env:`-sourced refs the client already resolved against
   * *its own* environment and forwarded by value, keyed by `SecretRef.name`.
   * A match there is used as-is instead of re-resolving against this
   * `SecretResolver`'s own (possibly frozen/stale) `env`, which is the fix
   * for the bug that started phase-20: a daemon spawned by one shell must not
   * silently answer `env:`-sourced secret lookups out of *its own* process
   * environment when the calling shell had a different one.
   */
  async resolveAll(
    refs: readonly SecretRef[],
    forwarded: readonly ResolvedSecret[] = [],
  ): Promise<ResolvedSecret[]> {
    const resolved: ResolvedSecret[] = [];
    for (const ref of refs) {
      const match = forwarded.find((s) => s.name === ref.name);
      resolved.push({ name: ref.name, value: match ? match.value : await this.resolveOne(ref) });
    }
    return resolved;
  }
