 ...
 *
 * This is the same write-then-rename pattern `OperatorRunStore.save`
 * (`apps/daemon/src/operator-run-store.ts`) already used ad hoc — extracted
 * here so `SessionStore`, `FileLock`, and any other `~/.windower` writer share
 * one implementation (`phase-20-daemon-optional.md` "Core primitives").
 *
