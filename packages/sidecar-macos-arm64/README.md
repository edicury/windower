...
pattern): npm resolves this package only on `darwin`/`arm64` hosts via its `os`/`cpu` fields, and
`packages/core`'s `resolveSidecarBinaryPath()` finds the binary at runtime via
`require.resolve("@windower/sidecar-macos-arm64/bin/windower-sidecar-macos")`.
...
```sh
cp native/macos/.build/release/windower-sidecar-macos packages/sidecar-macos-arm64/bin/windower-sidecar-macos
chmod +x packages/sidecar-macos-arm64/bin/windower-sidecar-macos
```

(`native/macos/.build/release/...` is produced by `swift build -c release --arch arm64` on an
...
users on arm64 would fail sidecar resolution at runtime with a clear "optional dependency not
