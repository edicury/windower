...
final class InputSynthesisTests: XCTestCase {
    ...
    /// The read-back half of this round trip — `EventTapSource.source(for:)`
    /// and `TimelineEventWire`'s `source` encoding — lives on the CAPTURE
    /// surface, so its coverage moved to
    /// `WindowerCaptureCoreTests/EventTapCaptureTests` in phase 21. What stays
    /// here is the write half: this target deliberately cannot see
    /// `WindowerCaptureCore` at all (see `Package.swift`), which is the point.
    func testStampingAnEventSetsTheSharedTagFieldTheCaptureSurfaceReadsBack() throws {
        guard let event = CGEvent(source: nil) else {
            throw XCTSkip("CGEvent creation unavailable in this environment")
        }
        XCTAssertEqual(WindowerEventTag.source(for: event), "user")
        ...
        XCTAssertEqual(event.getIntegerValueField(.eventSourceUserData), WindowerEventTag.magic)
        XCTAssertEqual(WindowerEventTag.source(for: event), "operator")
    }
