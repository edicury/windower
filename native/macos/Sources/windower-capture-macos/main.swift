...
// windower-capture-macos — the CAPTURE surface
...
// §Transport).
//
// Phase 21: this binary is the ONLY thing on the machine permitted to hold
// ScreenCaptureKit state. `performInput`/`resizeWindow` moved out to
// `windower-control-macos`, which links no ScreenCaptureKit at all — see
// Package.swift and scripts/check-no-screencapturekit.sh. Dispatching a
// control method here would silently recreate the coupling the split exists
// to remove, so those methods now fall through to the `default` branch and
// answer `UNSUPPORTED_CAPABILITY`, exactly as the protocol prescribes for a
// method this implementation does not advertise.
...
func handleRequest(id: JSONValue, method: String, params: JSONValue?) {
    do {
        ...
        case "getPermissions":
            let report = PermissionsService.captureReport(sidecarVersion: sidecarVersion)
...
func handle(line: String) {
    guard let data = line.data(using: .utf8) else {
        logStderr("windower-capture-macos: received non-UTF8 line, ignoring")
        return
    }
    ...
    do {
        ...
        logStderr("windower-capture-macos: failed to parse JSON-RPC line: \(error)")
        return
    }
    ...
    case .notification:
        ...
        logStderr("windower-capture-macos: ignoring unexpected notification: \(parsedLine.method ?? "?")")
    case .other:
        logStderr("windower-capture-macos: ignoring line that is neither request nor notification")
    }
}
...
// contracts/screen-capture-exclusivity.md §Process ownership: this process is
// a child of whatever holds ScreenCaptureKit exclusivity, and EOF is how it
// learns that owner is gone (the OS closes the pipe when the parent dies).
// Exiting here was already correct; exiting WITHOUT stopping the stream and
// finalizing the asset writer was not — it left a truncated, undecodable video
// behind. That cleanup is the entire orphan/truncation mechanism: no pid
// tracking, no reaper, no supervisor.
//
// Bounded on purpose. A wedged `SCStream.stopCapture`/`AVAssetWriter.finish`
// must not convert "the parent died" into "the child hangs forever holding
// ScreenCaptureKit" — the exact orphan this path exists to prevent. On expiry
// we log and exit anyway; a possibly-unfinalized file is strictly better than
// a live orphan.
let eofCleanupBudget = DispatchTimeInterval.seconds(10)

if CaptureSessionManager.shared.activeSessionCount > 0 {
    logStderr(
        "windower-capture-macos: stdin EOF with active capture — stopping and finalizing before exit"
    )
    let cleanupFinished = DispatchSemaphore(value: 0)
    DispatchQueue.global(qos: .userInitiated).async {
        CaptureSessionManager.shared.stopAllSessions()
        cleanupFinished.signal()
    }
    if cleanupFinished.wait(timeout: .now() + eofCleanupBudget) == .timedOut {
        logStderr(
            "windower-capture-macos: stdin EOF cleanup did not finish within its budget — exiting anyway"
        )
    }
}
