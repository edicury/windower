...
public final class CaptureSessionManager {
    ...
    /// Stops and finalizes every in-flight session — the one teardown caller
    /// that has no `sessionId` to name: `windower-capture-macos`'s stdin-EOF
    /// exit path. Per contracts/screen-capture-exclusivity.md §Process
    /// ownership, the capture process is owned by whoever spawned it, and when
    /// that parent dies the OS closes the pipe; before exiting on that EOF the
    /// process MUST stop any active capture and finalize any in-progress
    /// output, so a dead parent leaves a decodable file behind instead of a
    /// truncated one. This is the whole of the orphan/truncation mechanism —
    /// there is deliberately no pid tracking, reaper, or supervisor anywhere.
    ///
    /// Deliberately a thin loop over `stopCapture` rather than its own
    /// teardown: that ordering (microphone stop, event-tap stop, liveness-timer
    /// cancel, `SCStream.stopCapture`, then an awaited `writer.finish`) is
    /// subtle and must not be re-derived in a second place.
    ///
    /// Never throws. EOF means the process is already on its way out, so a
    /// session that fails to finalize is logged to stderr (stdout stays
    /// protocol-data-only) and the remaining sessions are still attempted.
    /// Callers on the EOF path must additionally bound how long they wait for
    /// this, so a wedged `SCStream`/`AVAssetWriter` can't turn "the parent
    /// died" into "the child hangs forever".
    ///
    /// - Returns: the session ids it attempted, in the order attempted.
    @discardableResult
    public func stopAllSessions() -> [String] {
        lock.lock()
        ...
        for sessionId in ids {
            do {
                let result = try stopCapture(params: StopCaptureParams(sessionId: sessionId))
                logStderr(
                    "windower-capture-macos: stdin EOF — finalized session \(sessionId) to "
                        + "\(result.outputFilePath) (\(Int(result.actualDurationMs.rounded()))ms)")
            } catch {
                logStderr(
                    "windower-capture-macos: stdin EOF — failed to finalize session "
                        + "\(sessionId): \(error)")
            }
        }
        return ids
    ...
    private func emitLog(sessionId: String, level: String, message: String) {
        logStderr("windower-capture-macos: \(message)")
        guard let handler = onNotification else { return }
