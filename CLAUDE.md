...
- **Windower never drives UI itself** — driving is always the calling agent's job, via its own tools (computer-use, a browser skill, or manual scripting). Windower ships no code that synthesizes input or perceives the screen for decision-making; it only records, reports targets, and resizes windows.
