...
**Historical note (Phase 24):** the bug that motivated this phase — `windower operate` dying with `OPERATOR_MISSING_API_KEY` when invoked from a shell that had the key, because the daemon's environment was frozen at spawn time from whichever shell first won the spawn race — lived in `windower operate`'s env handling. The Operator no longer exists (Phase 24), so that specific symptom is moot. The daemon-lifecycle hardening this phase produced (version handshake, spawn lockfile, graceful shutdown, cross-process safety) stands independent of the Operator and is unaffected by its removal; only the operator-specific bullets below (blocking `operate`, `operate --detach`/`abort`, secret-scoped `hello` env, operator-run store extraction) are historical.
...
- **Daemon-optional.** `record`, `targets`, `doctor`, `permission`, `resize`, `status`, `list`, `config` run with **no daemon**. The daemon auto-starts only for `start`/`stop`/`cancel`.
...
- 🔵 `src/fs/atomic-write.ts` — `writeFileAtomic` (temp + rename), the pattern `apps/daemon/src/session-store.ts` already uses.
...
- 🔵 `hello` handshake in `ensureDaemonRunning`: connect → `hello` → compare protocol versions → on match return; on mismatch, restart **if safe**, at most once, never looping.
  - Safe means: `list_sessions({state:"recording"})` is empty. If anything is in flight, refuse with `DAEMON_BUSY` naming the active ids and pointing at `windower stop <id>` / `windower daemon restart --force`.
...
- 🔵 `checkIdle` (`server.ts:115-127`) must count active sessions accurately so nothing in flight is idle-shutdown out from under itself.
...
- 🔵 Move, largely verbatim: `session-store`, `output-resolver`, `event-timeline-writer`, `narration-mux`, `passthrough`.
...
- 🔵 `RecordingEngine` = today's `SessionManager` minus the process-local `activeTargetKeys` guard, plus `TargetLock` and the extracted manifest module. Its `activeSidecars`/`eventWriters`/`plannedOutputPaths` maps are per-instance and work unchanged in both hosts — a `record --duration` CLI process holds one instance for its whole life.
...
- 🔵 `status` / `list` read the stores directly; `stop` / `cancel` / `daemon status` / `daemon stop` use `attach`.
...
- 🔵 Same policy table. `check_permissions`, `list_targets`, `resize_window`, `get_session`, `list_sessions` go `local`.
...
- 🔵 `plugins/claude-code/SKILL.md` — daemon-optional guidance for `record`/`start`/`stop`.
- 🔵 **A central environment-variable reference** — none exists today. `WINDOWER_HOME`, `WINDOWER_SIDECAR_BINARY_PATH`, `WINDOWER_DAEMON_BIN_PATH`, `WINDOWER_BACKEND` are currently documented across three files or not at all.
...
- 🔵 `record` end-to-end, including Ctrl-C mid-run finalizing the recording.
...
- A stale daemon from an older version is detected and restarted automatically when idle, and refused with an actionable `DAEMON_BUSY` when a recording is live — never silently driven.
