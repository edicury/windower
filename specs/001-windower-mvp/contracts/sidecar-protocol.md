...
|---|---|---|
| **Capture** | `describe`, `enumerateTargets`, `getPermissions`, `requestPermission`, `startCapture`, `stopCapture`, `cancelCapture` | `windower-capture-macos` |
| **Control** | `describe`, `getPermissions`, `requestPermission`, `resizeWindow` | `windower-control-macos` |

Why each method lands where it does:

- `enumerateTargets`, `startCapture`, `stopCapture`, `cancelCapture` all read screen content or screen-content metadata, so they belong to whatever owns the platform's screen-capture state. There are no separate audio methods to place — audio is configured entirely through `startCapture`'s `AudioSettings` param, so it follows `startCapture` onto the capture surface.
- `resizeWindow` writes window geometry. It reads no screen content and shares no OS resource with capture, so it is on the control surface.
...
**This grouping is not a breaking change.** No method's params or result shape changes as part of the split. Capability strings already work per-implementation via `describe` with zero new fields — the protocol never assumed one process implements every method, it just happened to be true until Phase 21.
